    ...
    for (let right_index = 1; right_index < tiles.length; right_index++) {
        ...
        if (isJoker(current_tile) && isJoker(next_tile)) {
            ...
            if (getTileValue(copy) === 0) { // computed value will be zero only if -> [...J 1...]
                // Standard rules: a joker cannot stand before 1 mid-run (no wrap 13→1).
                return null
            }
            freezed.push(copy)
            ...
            freezed.push(current_tile)
            if (right_index === tiles.length - 1) {
                // Standard rules: a joker after 13 is invalid (run ends at 13).
                if (getTileValue(current_tile) === 13) {
                    return null
                }
                let copy = setTileValue(next_tile, getTileValue(current_tile) + 1)
    ...
    if (isSameColor(tiles)) {
        ...
        for (let right = 1; right < freezed.length; right++) {
            ...
            if (getTileValue(next) - getTileValue(curr) === 1) {
                ...
                // Standard rules: a run ends at 13 — 13→1 wrap-around is NOT allowed.
                return 0
            } else {
                return 0
            }
